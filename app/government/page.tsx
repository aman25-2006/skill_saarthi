            ...
            <div className="grid grid-cols-4 gap-1 py-1.5 sm:hidden">
              {govTabDefinitions
                ...
                .map((tab) => {
                  ...
                  const mobileLabel = 
                    tab.id === 'ai-insights' ? (language === 'hi' ? 'एआई इनसाइट' : language === 'mr' ? 'एआय इनसाइट' : 'AI Insights') :
                    tab.id === 'students' ? (language === 'hi' ? 'छात्र' : language === 'mr' ? 'विद्यार्थी' : 'Students') :
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center py-2 px-1 rounded-xl text-center border transition-all ${
                  govTabDefinitions.some((t) => t.id === activeTab && !t.primaryMobile)
                    ? 'border-saffron bg-orange-50 dark:bg-amber-950/60 text-saffron dark:text-amber-300 font-bold shadow-xs'
                    : 'border-gray-200 dark:border-slate-700/80 bg-slate-50 dark:bg-slate-800/60 text-slate-700 dark:text-slate-300'
                }`}
                aria-label="More Options"
              >
                <MoreHorizontal size={16} className={govTabDefinitions.some((t) => t.id === activeTab && !t.primaryMobile) ? 'text-saffron dark:text-amber-300' : 'text-slate-500 dark:text-slate-400'} />
